...
def datacollected():
    ...
    header = ['serial_number','countries' , 'total_cases' , 'new_cases' , 'total_death' , 'recovered', 'total_recovered','active_cases',
            'serious_critical' , 'total_cases_permn', 'total_deaths_permn', 'total_tests', 'total_test_permillion', 'total_pop' ]
    for i in abc:
        id = i.find_all('td')
        if(id[1].text.strip().lower() == countrynotification):
            ...
            recovered = id[6].text.strip()
            notification("CORONA RECENT UPDATES OF  {}".format(countrynotification.upper()),
                        "Total Cases : {}\nTotal Deaths : {}\nNew Cases : {}\nRecovered : {}".format(
                           totalcases1,totaldeath,newcases,recovered 
                        ))
    ...
    dataframe = pd.DataFrame(list(zip(serial_number,countries , total_cases , new_cases , total_death , 
                    recovered, total_recovered,active_cases,
...
# create message box
        if(len(flist) != 0):
            messagebox.showinfo("Notification","Corona Record is saved {}".format(path2),parent =covid)
...
covid = Tk()
covid.title("Corona Virus Information")
covid.geometry('800x500+200+80')
covid.configure(bg='black')
covid.iconbitmap('C:\\Users\\Aakash Garg\\Desktop\\Github\\Python Projects\\Covid-19 Cases Notificator\\favicon.ico')    # download only ico files 
flist = []
path = ''

#### Labels
mainlabel = Label(covid,text="Corona Virus Live Tracker",font=("new roman",30,"italic bold underline"), bg = "black",width=33
                        ,fg = "white",bd=5)
mainlabel.place(x=0,y=0)


label1 = Label(covid,text="Country Name",font=("arial",20,"italic bold"), bg = "#046154")
label1.place(x=15,y=100)

label2 = Label(covid,text="Download File in ",font=("arial",20,"italic bold"), bg = "#046154")
label2.place(x=15,y=200)

cntdata = StringVar()
entry1 = Entry(covid,textvariable = cntdata ,font = ("arial",20,"italic bold"), relief = RIDGE,bd = 2 , width = 22)
entry1.place(x = 380, y = 100)

#### BUTTONS

Inhtml = Button(covid,text = "Html", bg = "#2DAE9A", font = ("arial",15,"italic bold"),relief = RIDGE,activebackground = "#05945B",
                activeforeground = "white",bd = 5,width = 5,command = inhtmldownload)
Inhtml.place(x = 300, y = 200)

Injson = Button(covid,text = "json", bg = "#2DAE9A", font = ("arial",15,"italic bold"),relief = RIDGE,activebackground = "#05945B",
                activeforeground = "white",bd = 5,width = 5,command = injsondownload)
Injson.place(x = 300, y = 260)

Inexcel = Button(covid,text = "Excel", bg = "#2DAE9A", font = ("arial",15,"italic bold"),relief = RIDGE,activebackground = "#05945B",
                activeforeground = "white",bd = 5,width = 5,command = inexceldownload )
Inexcel.place(x = 300, y = 320)

Submit = Button(covid,text = "Submit", bg = "#CB054A", font = ("arial",15,"italic bold"),relief = RIDGE,activebackground = "#7B0519",
